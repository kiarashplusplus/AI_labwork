...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        returns: number (float or int)
        """
        p = zip(self.my_inputs, self.my_weights)
        return 1.0 / (1.0 + math.e**(-1 * sum([x.output()*y.get_value() for (x,y) in p])))
    ...
    def compute_doutdx(self, elem):
        ...
        if self.has_weight(elem):
            dzdw = self.my_inputs[self.my_weights.index(elem)].output()
            return self.output() * (1 - self.output()) * dzdw
        c = 0
        for w in self.my_weights:
            if self.isa_descendant_weight_of(elem, w):
                c += self.my_inputs[self.my_weights.index(w)].dOutdX(elem) * w.get_value()
        return self.output() * (1 - self.output()) * c
...
class PerformanceElem(DifferentiableElement):
    ...
    def output(self):
        ...
        returns: number (float/int)
        """
        return ((self.my_desired_val - self.my_input.output())**2)/2.0
