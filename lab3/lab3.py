...
def focused_evaluate(board):
  ...
  #  return basic_evaluate(board)
    ...
    other=board.longest_chain(board.get_other_player_id())
...
def color(board):
    ...
    if board.num_tokens_on_board()%2 is 0:
        return 1
    else: 
        return -1

def helperFunc(board, depth, eval_fn, get_next_moves_fn=get_all_next_moves,
		      is_terminal_fn=is_terminal):
              
    if board.alpha >= board.beta:
        return board.alpha
    
    if is_terminal_fn(depth, board) is True or depth is 0:
        return eval_fn(board)
    else:
        all_children=get_next_moves_fn(board)
        
        for move, child in all_children:
            child.alpha=-board.beta
            child.beta = -board.alpha
            val= -1*helperFunc(child, depth-1, eval_fn, get_next_moves_fn, is_terminal_fn)
    ...
    for row in range(6):
        for col in range(7):
            if board.get_cell(row, col) == player:
                score = score - (abs(3 - col) + abs(3 - row)) * 10
...
WHAT_I_FOUND_INTERESTING = "the ideas of evaluation"
WHAT_I_FOUND_BORING = "syntax"
NAME = "Kiarash"
EMAIL = "kiarash@mit.edu"
